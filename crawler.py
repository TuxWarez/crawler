...
deadly_floor = [0, 1, 2]; pure_deadly_floor = 2; pure_deadly_floor_plus = 3; spike_tic = 0
x_coord = 9; y_coord = 8
coords = np.array([[9, 8], [7, 7], [1, 10], [1, 12], [2, 10], [5, 13], [7, 11], [13, 11], [2, 13], [6, 8]])
coords_static = copy.deepcopy(coords)
tic = True; space = " "
...
def color_check(iterator):
    ...
    elif iterator == '+': color = bcolors.YELLOW
...
def print_level():
    ...
    for i in range(len(lvl01)):
        for j in range(len(lvl01)):
            if j + 1 < 15 and LEVELS[map_index][i, j + 1] in EXTENDABLE:
                rspace = bcolors.WHITE + "═" + bcolors.ENDC
            else:
                rspace = " "
            ...
            if dots == False:
                if LEVELS[map_index][i, j] == ".":
                    ...
                    print(text + LEVELS[map_index][i, j] + bcolors.ENDC, end=rspace)
            else:
                print(text + LEVELS[map_index][i, j] + bcolors.ENDC, end=rspace)
        print()
...
def character_movement(x_dir, y_dir):
    global key_ammount, map_index, y_coord, x_coord, move_count, all_moves, latch_1, latch_2, latch_3, tic, noclip, god, retry_count, latch_arr, switch_count, spike_tic, coords, coords_static
    move_count += 1
    if (LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] in WALLS or LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "o") and noclip == 0:
        move_count -= 1
        tic = False
        return
    elif LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "D":
        if key_ammount == 0 and noclip == 0:
            move_count -= 1
            tic = False
            return
        key_ammount -= 1
    elif LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "B" and noclip == 0:
        while True:
            ...
            break
    elif LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "K" or LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "k":
        key_ammount += 1
    elif LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] in DEADLY and god == 0:
        ...
        print_level()
        return
    elif LEVELS[map_index][(y_coord + y_dir) % 15, (x_coord + x_dir) % 15] == "E" and noclip == 0:
        pure_deadly_floor = 2
        LEVELS[map_index] = copy.deepcopy(LEVELS_START[map_index])
        map_index += 1
    ...
    if x_coord == -1: x_coord = 14
    elif x_coord == 15: x_coord = 0
    if y_coord == -1: y_coord = 14
    elif y_coord == 15: y_coord = 0
    LEVELS[map_index][y_coord, x_coord] = "@"
    if LEVELS[map_index][(y_coord - y_dir) % 15, (x_coord - x_dir) % 15] not in INDESTRUCTABLE:
        LEVELS[map_index][(y_coord - y_dir) % 15, (x_coord - x_dir) % 15] = "."
    if noclip == 1 and noclip_destroy == 0:
         LEVELS[map_index][(y_coord - y_dir) % 15, (x_coord - x_dir) % 15] = LEVELS_START[map_index][(y_coord - y_dir) % 15, (x_coord - x_dir) % 15]
    tic = True

def character_input():
    global key_ammount, map_index, y_coord, x_coord, move_count, tic, showpos, noclip, noclip_destroy, god, space, dots, retry_count, latch_1, latch_2, latch_3, latch_arr, all_moves
    action = getch.getch()
    if action == "r" or action == "R":
        ...
        print()
        print("To change level type in 'changelvl ##' with ## being the level number")
        ...
        try:
            ...
            if command[0].lower() == "changelvl":
                ...
                if len(command) < 2:
                    ...
                    return
                dots = int(command[1])
            elif command[0].lower() == "win" or command[0].lower() == "player.win":
                LEVELS[map_index] = copy.deepcopy(LEVELS_START[map_index])
                map_index += 1
...
def map_specific_events():
    global map_index, move_count, latch_1, deadly_floor, absolute_deadly_floor, pure_deadly_floor, absolute_deadly_floor_plus, pure_deadly_floor_plus, x_coord, y_coord, tic, latch_2, latch_3, spike_tic, dots
    if map_index == 3:
        ...
        LEVELS[map_index][11, 1 + ((spike_tic - 1) % 11)] = LEVELS_START[map_index][11, 1 + ((spike_tic - 1) % 11)]
        LEVELS[map_index][11, 1 + (spike_tic % 11)] = "*"
        LEVELS[map_index][11, 1 + ((spike_tic + 1) % 11)] = LEVELS_START[map_index][11, 1 + ((spike_tic + 1) % 11)]
        LEVELS[map_index][11, 1 + ((spike_tic + 2) % 11)] = "*"
        LEVELS[map_index][9, 13 - ((spike_tic + 2) % 13)] = LEVELS_START[map_index][9, 13 - ((spike_tic + 2) % 13)]
        LEVELS[map_index][9, 13 - ((spike_tic + 3) % 13)] = "*"
        LEVELS[map_index][9, 13 - ((spike_tic + 4) % 13)] = LEVELS_START[map_index][9, 13 - ((spike_tic + 4) % 13)]
        LEVELS[map_index][9, 13 - ((spike_tic + 5) % 13)] = "*"
        LEVELS[map_index][7, 1 + ((spike_tic + 3) % 13)] = LEVELS_START[map_index][7, 1 + ((spike_tic + 3) % 13)]
        LEVELS[map_index][7, 1 + ((spike_tic + 4) % 13)] = "*"
        LEVELS[map_index][7, 1 + ((spike_tic + 5) % 13)] = LEVELS_START[map_index][7, 1 + ((spike_tic + 5) % 13)]
        LEVELS[map_index][7, 1 + ((spike_tic + 6) % 13)] = "*"
        LEVELS[map_index][5, 13 - ((spike_tic + 3) % 13)] = LEVELS_START[map_index][5, 13 - ((spike_tic + 3) % 13)]
        LEVELS[map_index][5, 13 - ((spike_tic + 4) % 13)] = "*"
        LEVELS[map_index][5, 13 - ((spike_tic + 5) % 13)] = LEVELS_START[map_index][5, 13 - ((spike_tic + 5) % 13)]
        LEVELS[map_index][5, 13 - ((spike_tic + 6) % 13)] = "*"
        LEVELS[map_index][3, 1 + ((spike_tic + 7) % 13)] = LEVELS_START[map_index][3, 1 + ((spike_tic + 7) % 13)]
        LEVELS[map_index][3, 1 + ((spike_tic + 8) % 13)] = "*"
        LEVELS[map_index][3, 1 + ((spike_tic + 9) % 13)] = LEVELS_START[map_index][3, 1 + ((spike_tic + 9) % 13)]
        LEVELS[map_index][3, 1 + ((spike_tic + 10) % 13)] = "*"
        if LEVELS[map_index][0, 1] == "*" and latch_arr[0] == 0:
            ...
            latch_arr[6] = 1
            print_level()
        if x_coord == 1 and y_coord == 2:
            coords[map_index] = [1, 2]
            LEVELS[map_index][2, 1] = "@"
            LEVELS[map_index][11, 13] = "."
        spike_tic += 1
    elif map_index == 8:
        LEVELS[map_index][13, 4 + ((spike_tic - 2) % 9)] = "."
        LEVELS[map_index][13, 4 + (spike_tic % 9)] = "*"
        LEVELS[map_index][13 - ((spike_tic + 6) % 13), 13] = "."
        LEVELS[map_index][13 - ((spike_tic + 8) % 13), 13] = "*"
        LEVELS[map_index][1, 13 - ((spike_tic + 5) % 13)] = "."
        LEVELS[map_index][1, 13 - ((spike_tic + 7) % 13)] = "*"
        LEVELS[map_index][1 + (spike_tic % 11), 1] = "."
        LEVELS[map_index][1 + ((spike_tic + 2) % 11), 1] = "*"
        LEVELS[map_index][11, 1 + ((spike_tic + 4) % 11)] = "."
        LEVELS[map_index][11, 1 + ((spike_tic + 6) % 11)] = "*"
        LEVELS[map_index][11 - ((spike_tic + 4) % 9), 11] = "."
        LEVELS[map_index][11 - ((spike_tic + 6) % 9), 11] = "*"
        LEVELS[map_index][3, 11 - ((spike_tic + 2) % 9)] = "."
        LEVELS[map_index][3, 11 - ((spike_tic + 4) % 9)] = "*"
        LEVELS[map_index][3 + ((spike_tic + 4) % 7), 3] = "."
        LEVELS[map_index][3 + ((spike_tic + 6) % 7), 3] = "*"
        LEVELS[map_index][9, 3 + ((spike_tic + 2) % 7)] = "."
        LEVELS[map_index][9, 3 + ((spike_tic + 4) % 7)] = "*"
        if x_coord == 6 and y_coord == 7:
            coords[map_index] = [6, 7]
            LEVELS[map_index][7, 6] = "@"
            LEVELS[map_index][13, 2] = "."
        spike_tic += 2
    elif map_index == 9:
